...
class NaiveBayes:
    class Splitter:
        ...
        """
        def __init__(self): # add attributes and initialize
            self.train = []
            self.test = []
            self.dev = []
    ...
    def __init__(self):
        ...
        self.V = -1
        self.T_pos = -1 # true positive
        self.T_neg = -1 # true negative
    ...
    #
    # We have provided you with a Naive Bayes Classfier. You can use it as is or replace it with your solution from PA2.
    #
    # If the BOOLEAN_NB flag is true, your methods must implement Boolean (Binarized)
    # Naive Bayes (that relies on feature presence/absence) instead of the usual algorithm that relies on feature counts.
    #
    # If the USE_BIGRAMS flag is true, your methods must use bigram features instead of the usual bag-of-words (unigrams)
    #
    # If any one of the BOOLEAN_NB or USE_BIGRAMS flags is on, the 
    # other is meant to be off.
    
    # self is the training dataset
    def classify(self, words):
        total_docs = sum(list(self.num_docs.values()))
        logPriorPos = math.log(float(self.num_docs['pos']) / total_docs)
        logPriorNeg = math.log(float(self.num_docs['neg']) / total_docs)
        if self.T_pos == -1:
            self.T_pos = sum(list(self.counts_pos.values())) # the number of words in pos
        if self.T_neg == -1:
            self.T_neg = sum(list(self.counts_neg.values())) # the number of words in  neg
        if self.V == -1:
            self.V = len(set(list(self.counts_pos.keys()) + list(self.counts_neg.keys()))) # The number of unrepeated tokens in training dataset
        ...
        seen = set()
        if self.USE_BIGRAMS: # use bigram features
            words = ['<s>'] + words + ['</s>']
...
# END TODO(Modify code beyond here with caution)#######################################################################################
    ...
    def buildSplit(self,include_test=True): # default include_test is true
...
class negativetrans():
    ...
    def trans(self):
        point = 0
        while point <= len(self.content):
            star = self.content.find('n\'t')
            self.content = self.content.replace('n\'t', 'n\'\'t',1)
            a = self.content[star:].find(',')
            b = self.content[star:].find('.')
            c = self.content[star:].find('!')
            if a+b+c != -3 and star != -1:
                end = min([n for n in [a,b,c] if n>=0]) + star
                #if end - star > 2:
                self.content = self.content[0:star] + self.content[star:end].replace(' ', ' NOT_') + self.content[end:]
                point = self.content.rfind('NOT_')+3
            else:
                point = len(self.content)+1
        self.content.replace('n\'\'t', 'n\'t')
        return self.content

#trabs = negativetrans()
#trabs.content = "I don't like this book, she doesn't like it also."
#trabs.trans()
